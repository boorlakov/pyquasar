...
parser.add_argument("--test_approx", type=str)
parser.add_argument("--train_approx", type=str)
...
test_approx_datapath = args.test_approx
train_approx_datapth = args.train_approx
...
train_approx_data = np.load(train_approx_datapth)

approx_grad_x = train_approx_data["grad_x"]
approx_grad_y = train_approx_data["grad_y"]
approx_grad_z = train_approx_data["grad_z"]

train_approx_data.close()

grad = np.concatenate([grad_x - approx_grad_x, grad_y - approx_grad_y, grad_z - approx_grad_z])
...
test_approx_data = np.load(test_approx_datapath)

test_approx_true_grad_x = test_approx_data["grad_x"]
test_approx_true_grad_y = test_approx_data["grad_y"]
test_approx_true_grad_z = test_approx_data["grad_z"]

test_approx_data.close()
...
for iter_n in [100000]:
  ...
  surface_diff_x = surface_test_grad_x + approx_grad_x - grad_x
  surface_diff_y = surface_test_grad_y + approx_grad_y - grad_y
  surface_diff_z = surface_test_grad_z + approx_grad_z - grad_z
  ...
  diff_x = test_grad_x + test_approx_true_grad_x - test_true_grad_x
  diff_y = test_grad_y + test_approx_true_grad_y - test_true_grad_y
  diff_z = test_grad_z + test_approx_true_grad_z - test_true_grad_z
